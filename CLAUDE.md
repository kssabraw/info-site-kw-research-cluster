...
| Embeddings | **OpenAI `text-embedding-3-small` @ 1536-dim** (the original calibrated lock — **restored as the live provider 2026-06-16**). The 2026-06-15 owner override to **Google Gemini Embedding 2** (`gemini-embedding-2-preview`) was deployed + flipped live, but **rolled back** the next day: in write-time calibration, Gemini gave **poor relevance discrimination** — off-topic keywords passed the gate even at `relevance_threshold` 0.90 (cosines compressed too high / not separating topics; likely the `SEMANTIC_SIMILARITY` task type, possibly preview-model behavior). Rollback = `EMBEDDING_PROVIDER=openai` (one env var; no code change, no recalibration — OpenAI's `0.65` etc. thresholds are unchanged). **The provider/model-pluggable embedder stays in place (dormant)** — `GeminiEmbedder`, the per-session `embedding_model` 409 guard, and `GET /debug/embedding-health` are kept. Revisit Gemini when Embedding 2 is GA and/or with `RETRIEVAL_*` task types (needs a re-embed + threshold recalibration); 1536-dim keeps the `vector(1536)` schema either way. Vector spaces must never be mixed across providers/models. |
...
| `docs/aio-optimization-plan.md` | The **AIO + ChatGPT (answer-engine) optimization** plan/working-notes for the M13 Brief Generator (owner decisions 2026-06-17): the §0 **answer-engine-first** pivot (organic = floor; **flagged PRD divergence**), **full MCS** replacing organic heading selection, dual/triple-space embeddings (Gemini 2 for AIO proximity, 3-large for gates + ChatGPT proximity), gap + collision analysis, and the source research verbatim. Read before any AIO/heading work on M13. |
...
| 1.20 | 2026-06-23 | **Articles library + 3 publish/save destinations DONE — deployed (owner decision: written content must "live somewhere" — in-app, Google Drive, or a GitHub repo; explicitly NOT auto-pushed to a live site).** Three PRs on `claude/intelligent-keller-6sx8ho`. **(1) In-app library (PR #40, first commit):** owner **Articles tab** + `GET /sessions/{id}/articles` (latest article per cluster — name/words/cost/source/date, paged above the ~1000-row cap); `ArticlePanel` gained a **read-only** mode (loads the stored article via GET instead of regenerating) + **Copy Markdown** / **Download .md**; **Download-all (.zip)** of every article's Markdown via the existing `csv-snapshots` bucket + signed URL. Articles already lived in `fanout.article_outputs`; this is the browse/read/export surface. **(2) GitHub publish (PR #40):** per-article commit + **Push-all** (one commit via the Git **Trees** API regardless of count) as **Astro content Markdown** at `{content_path}/{silo-slug}/{article-slug}.md` — the SAME slug/silo structure the M15 internal links use, so links resolve on the built site. `app/writer/publish/markdown.py` (pure: YAML frontmatter title/description/pubDate/slug/silo/draft; lifts the leading H1 into the title; derives a description — tested) + `github.py` (httpx Contents API create-or-update + Trees bulk; no new dep). `GITHUB_PUBLISH_TOKEN` env (fine-grained PAT, Contents:write). **(3) Google Drive save (PR #40):** per-article → a formatted **Google Doc** (Drive's HTML→Doc conversion of the article's `article_html`). Personal Gmail has **no Shared Drives**, so it's **OAuth-as-user** (`drive.file` scope) not a service account — `GOOGLE_OAUTH_CLIENT_ID/SECRET/REFRESH_TOKEN` env (publish the consent screen to *production* so the refresh token doesn't expire after 7 days). `app/writer/publish/drive.py`; deps `google-api-python-client` + `google-auth`. Bulk-Drive deferred (no one-shot multi-file commit like GitHub's tree). **Wiring:** `sessions.publish_config` jsonb (migration `20260626000000_publish_config.sql`, applied to prod) holds per-session GitHub repo/branch/path + Drive folder id; `GET /sessions` returns `publish_config` + `publish_available {github, drive}` (booleans from env presence, **no secrets**) so the Articles tab shows GitHub/Drive buttons only once their server creds are set; **Publish settings** card (GitHub + Drive folder). All destinations **dormant until creds provisioned** — deploy `a0cc3e20` SUCCESS, prod logs clean (new Google deps imported, scheduler still running). The in-app library works now; GitHub/Drive await the owner setting `GITHUB_PUBLISH_TOKEN` / `GOOGLE_OAUTH_*`. Worked on `claude/intelligent-keller-6sx8ho`. |
