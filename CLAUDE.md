...
| Embeddings | **Google Gemini `gemini-embedding-2-preview` (Embedding 2, public preview, #1 MTEB, multimodal) @ 1536-dim (Matryoshka)** via AI Studio (`GEMINI_API_KEY`) — owner override 2026-06-15 (whole-app, quality/consistency; supersedes the prior OpenAI `text-embedding-3-small` lock). **LIVE in prod** (`EMBEDDING_PROVIDER=gemini` + `GEMINI_EMBEDDING_MODEL=gemini-embedding-2-preview` on the Railway service, health-verified 2026-06-15). Provider+model pluggable via config; 1536-dim keeps the `vector(1536)` schema; vector spaces must never be mixed across providers/models (per-session `embedding_model` 409 guard). **Preview-model risk:** a model change/deprecation shifts the space → re-embed + recalibrate. **The 8 cosine thresholds remain OpenAI-calibrated — recalibration on live Embedding-2 runs is IN PROGRESS; clustering quality is degraded until locked.** GA fallback: `gemini-embedding-001`. |
