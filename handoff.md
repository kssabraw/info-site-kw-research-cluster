...
_Last updated: 2026-05-26. M6 (§7.11 site architecture) **complete, signed off, merged to `main`** — validated live on `retatrutide` `4ecefaa1`. M7 (Owner UI) is next._
...
**M6 is done and signed off. Next is M7 — Owner UI (PRD §15.1 / §9):** the three views (Table §9.1, Cluster §9.2, Architecture §9.3) + cluster-editing operations + the Project/Session Browser (§9.4, which finally gives the UI session-resume that calibration has been working around). Stop for a human go-ahead before building (milestone discipline). M6's `GET /sessions/{id}/architecture` is the read-only feed for the two-panel Architecture View; `GET /summary`'s `architecture` block tells the UI whether an architecture exists / when it was generated.

**Decision to settle at/before M7** (carried from RF/M5, still open): the **orchestrator-vs-direct planner default** (orchestrator default, direct via `{"direct": true}`). The validated `4ecefaa1` was planned in direct mode (315 articles).
