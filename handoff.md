...
**M6 is implemented on `claude/gifted-clarke-pONCI` but needs three things before sign-off:** (1) **apply the migration** `20260526000000_site_architecture.sql` to the live DB (via Supabase MCP `apply_migration` or `supabase db push` — it's a single new table + RLS, no data change); (2) **live-validate** on a `complete` session (e.g. re-plan `retatrutide` `4ecefaa1`, then `POST /architecture`, poll `/summary` until `architecture` non-null, `GET /architecture`) — checks the Opus pillar prose and that the deterministic links look right; (3) **merge to `main`** once validated. Then M7 (Owner UI) is the PRD-sequence next.
