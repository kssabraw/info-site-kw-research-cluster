...
- **M11 — Cost confirmation + observability (PRD §15.1 / §16):** ✅ **complete, merged to `main`, and deployed** (built on `claude/exciting-davinci-tZGwH`; meter confirmed working in prod 2026-06-09 — only the §15.3 live-validation checklist in §2 remains). Real-metered per-step cost → `actual_cost_usd` + new `cost_breakdown` jsonb, flushed live every 10s from the jobs (`app/cost_meter.py` + `app/cost_attribution.py`); `ContextThreadPoolExecutor` (`app/concurrency.py`) propagates the meter + correlation/session ids into nested pipeline threads; the four external-call sites populate `cost_usd`; live cost banner on `/summary` (`shared/CostBanner.tsx`) on Owner + VA; owner-only `GET /sessions/{id}/debug` + `owner/DebugView.tsx` (clustering + orchestrator logs + cost). Migration `20260529000000_session_cost_breakdown.sql` applied live via MCP. 176 backend tests + ruff clean; frontend builds. See `CLAUDE.md` "Active milestone" for full decisions/divergences. **With M11 the M1–M11 build sequence is done — no next milestone; remaining work is live validation (§2).**

## 2. Immediate next action (resume here)

**M11 is merged to `main` and deployed** (Railway redeployed 2026-06-09 02:12 UTC
on latest `main`; the M11 meter is confirmed working in prod — 3 sessions carry real
`actual_cost_usd` + per-phase `cost_breakdown`). The `cost_breakdown` migration was
applied to the live DB via MCP, so the deploys are schema-consistent. **With M11, the
M1–M11 build is complete — there is no next milestone to start.** What remains is the
§15.3 Definition-of-Done live validation that the sandbox can't run (no egress).
(Earlier dated entries below that say M11 is "not yet merged" are point-in-time
history — superseded by the 2026-06-09 entries at the top of this file.)
...
2. On the completed session, confirm `actual_cost_usd` lands **within ±25% of the
   §8.1 standard estimate (~$2.80)** — §15.3 #7. (**§7.8 metrics enrichment is now
   built** — migration `keyword_metrics` shipped 2026-05-29 — so the "+metrics" line
   *can* now be exercised; the earlier "can't be exercised" note is stale.) If LLM
   cost is wildly off,
...
```bash
. .venv/bin/activate
python -m pytest -q          # 297 tests passing (was 176 at M11; +post-M11 link-cap/cost/writer-ownership work)
